...
// Golden file test - EXACT comparison with numpy output
TEST_CASE("FFT matches numpy reference exactly", "[fft][golden]") {
    std::string goldenPath = test_utils::getGoldenDir() + "fft_golden.json";

    try {
        // Load expected input and output from numpy (1411-point FFT)
        auto expectedInput = test_utils::loadJsonArray(goldenPath, "test_cases.sine_440hz.input");
        auto expectedMagnitude = test_utils::loadJsonArray(goldenPath, "test_cases.sine_440hz.magnitude");

        REQUIRE(expectedInput.size() == MelConfig::FFT_SIZE);
        REQUIRE(expectedMagnitude.size() == MelConfig::FFT_SIZE / 2 + 1);
        ...
        // Use exact input from Python
        fft.forward(expectedInput.data(), output.data());
        fft.magnitude(output.data(), magnitude.data());

        // Compare EXACTLY (within float32 precision)
        for (size_t i = 0; i < magnitude.size(); ++i) {
            float diff = std::abs(magnitude[i] - expectedMagnitude[i]);
            INFO("Bin " << i << ": got " << magnitude[i] << ", expected " << expectedMagnitude[i]);
            REQUIRE(diff < 1e-4f);  // Allow minimal float32 rounding
        }
...
TEST_CASE("FFT impulse matches numpy exactly", "[fft][golden]") {
    std::string goldenPath = test_utils::getGoldenDir() + "fft_golden.json";

    try {
        auto expectedMagnitude = test_utils::loadJsonArray(goldenPath, "test_cases.impulse.magnitude");
        REQUIRE(expectedMagnitude.size() == MelConfig::FFT_SIZE / 2 + 1);

        FFT fft(MelConfig::FFT_SIZE);

        auto impulse = test_utils::generateImpulse(MelConfig::FFT_SIZE);
        std::vector<std::complex<float>> output(fft.getOutputSize());
        std::vector<float> magnitude(fft.getOutputSize());
