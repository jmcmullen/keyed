...
TEST_CASE("LogFilterbank weights match madmom", "[mel][filterbank][golden]") {
    std::string goldenPath = test_utils::getGoldenDir() + "filterbank.bin";

    try {
        // Load filterbank weights from madmom
        std::ifstream file(goldenPath, std::ios::binary);
        if (!file.is_open()) {
            throw std::runtime_error("Cannot open filterbank golden file");
        }

        int32_t numBins, numBands;
        file.read(reinterpret_cast<char*>(&numBins), sizeof(int32_t));
        file.read(reinterpret_cast<char*>(&numBands), sizeof(int32_t));

        REQUIRE(numBins == 705);
        REQUIRE(numBands == 136);

        std::vector<float> expectedWeights(numBins * numBands);
        file.read(reinterpret_cast<char*>(expectedWeights.data()),
                  expectedWeights.size() * sizeof(float));

        // Create our filterbank
        LogFilterbank fb(
            MelConfig::FFT_SIZE,
            MelConfig::SAMPLE_RATE,
            MelConfig::BANDS_PER_OCTAVE,
        ...
        // Compare weights by applying to unit vectors
        // For each FFT bin, check the output across all mel bands
        float maxDiff = 0;
        int maxDiffBin = 0, maxDiffBand = 0;

        for (int bin = 0; bin < numBins; ++bin) {
            // Create input with single bin set to 1
            std::vector<float> input(numBins, 0.0f);
            input[bin] = 1.0f;

            std::vector<float> output(numBands);
            fb.apply(input.data(), output.data());

            // Compare against expected
            for (int band = 0; band < numBands; ++band) {
                float expected = expectedWeights[bin * numBands + band];
                float diff = std::abs(output[band] - expected);
                if (diff > maxDiff) {
                    maxDiff = diff;
                    maxDiffBin = bin;
                    maxDiffBand = band;
        ...
        INFO("Max filterbank weight diff: " << maxDiff
             << " at bin " << maxDiffBin << ", band " << maxDiffBand);
        REQUIRE(maxDiff < 1e-5f);

    } catch (const std::exception& e) {
        WARN("Filterbank golden file error: " << e.what());
        SKIP("Filterbank golden file not available");
    }
}
...
// Helper function to test mel features against golden file
void testMelGoldenFile(const std::string& bpmStr) {
    std::string goldenPath = test_utils::getGoldenDir() + "mel_golden_" + bpmStr + "bpm.bin";
    std::string audioPath = test_utils::getAudioDir() + bpmStr + ".raw";

    // Load raw audio (float32)
    auto audio = test_utils::loadRawAudio(audioPath);
    INFO("Loaded " << audio.size() << " audio samples from " << bpmStr << ".raw");
    REQUIRE(audio.size() > 0);

    // Load expected features from Python (binary format)
    auto expected = test_utils::loadBinaryFeatures(goldenPath);
    INFO("Loaded " << expected.numFrames << " expected frames, "
         << expected.featureDim << " features each");
    REQUIRE(expected.featureDim == 272);

    // Test 50 frames for thorough validation
    const int NUM_TEST_FRAMES = std::min(50, expected.numFrames);
    const int FEATURE_DIM = expected.featureDim;

    // Process audio through C++ mel extractor
    StreamingMelExtractor extractor;
    std::vector<float> cppFeatures(FEATURE_DIM * 200);

    int numFrames = extractor.push(audio.data(), audio.size(),
                                   cppFeatures.data(), 200);

    INFO("C++ produced " << numFrames << " frames");
    REQUIRE(numFrames >= NUM_TEST_FRAMES);
    ...
    for (int f = 0; f < NUM_TEST_FRAMES; ++f) {
        ...
        for (int i = 0; i < FEATURE_DIM; ++i) {
            float cpp = cppFeatures[f * FEATURE_DIM + i];
            float python = expected.features[f * FEATURE_DIM + i];
            float diff = std::abs(cpp - python);
        ...
        INFO(bpmStr << " BPM - Frame " << f << ": max diff = " << maxDiff
             << " at index " << maxDiffIdx
             << " (C++=" << cppFeatures[f * FEATURE_DIM + maxDiffIdx]
             << " Python=" << expected.features[f * FEATURE_DIM + maxDiffIdx] << ")");

        // Require EXACT match (within float precision)
        REQUIRE(maxDiff < 1e-5f);
    }
}

// Golden file tests - EXACT comparison with Python madmom for ALL test files
TEST_CASE("MelExtractor matches madmom - 116 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("116");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
}

TEST_CASE("MelExtractor matches madmom - 120 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("120");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 125 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("125");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 126 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("126");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 131 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("131");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 132 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("132");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 134 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("134");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
...
TEST_CASE("MelExtractor matches madmom - 140 BPM", "[mel][golden]") {
    try {
        testMelGoldenFile("140");
    } catch (const std::exception& e) {
        WARN("Golden file error: " << e.what());
        SKIP("Golden files not available");
    }
